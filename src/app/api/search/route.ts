...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const endPoint = id
      ? `${apiUrl}/posts?search=${searchtext}&_embed&per_page=${perPage}&status=publish&page=${page}&categories=${id}`
      : `${apiUrl}/posts?search=${searchtext}&_embed&per_page=8&status=publish&page=${page}`;
